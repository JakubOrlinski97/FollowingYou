...
[].slice.call(folders).forEach(function (folder) {
  folder.addEventListener('click', function() {
    ...
    <section class="readme">
        <article><h1>Modules</h1><p>Modules provide easy integration with components. They can make a lot of complex things much simpler to other developers (such as adding 3D physics).</p>
<h3>Articles:</h3><ul>
<li><a href="https://hackernoon.com/migrating-to-whitestormjs-v2-beta-module-system-2eeaeda08a80">Migrating to WhitestormJS v2 beta. Module system</a></li>
</ul>
<h2>Simple module</h2><p>Let's create a simple module that will add <code>.alertRandom()</code> method to component, in which this module is used.</p>
<pre class="prettyprint source lang-js"><code>export default class SimpleModule {
  integrate() {
    this.alertRandom = function () {
      alert(Math.random());
    }
  }
}</code></pre><pre class="prettyprint source lang-js"><code>import SimpleModule from './SimpleModule';
...
sphere.alertRandom(); // will alert a random number.</code></pre><h2>Advanced module</h2><p>This example demonstrates the power of using modules.</p>
<pre class="prettyprint source lang-js"><code>export default class AdvancedModule {
  constructor(params = {}) {
...
}</code></pre><pre class="prettyprint source lang-js"><code>import {MeshComponent} from WHS;
...
myInstance.checkForColor(); // alerts &quot;color is the same&quot;</code></pre><h2>API</h2><h3><code>constructor()</code></h3><p>Used to handle input parameters that define module's behavior</p>
<h3><code>.integrate(self)</code></h3><p>In this method of any module, <code>this</code> is replaced with component's instance.
<code>integrate()</code> is executed once when component instance is created with <code>new</code> keyword.</p>
<p><code>self</code> - module scope.</p>
<h3><code>.postIntegrate(self)</code></h3><p>Same as <code>.integrate(self)</code>, but is invoked after <code>.build()</code>, that means you can use <code>.defer(() =&gt; {})</code> inside it.
Will be useful for working with models (<code>WHS.Importer</code>) and <code>.native</code> objects.</p>
<h3><code>.manager(manager)</code></h3><p>Allows components communicate with each other.
<code>manager</code> - is a ModuleManager instance provided by Component or App, where module is used</p>
<h3><code>.bridge</code></h3><ul>
<li>Object that handles functions called from component's code with <code>.applyBridge()</code> method.</li>
<li>Used to overwrite some parts of component.</li>
<li>Functions will be called only if specific component provides API for this &quot;bridge&quot; (using <code>.applyBridge({bridgeName: inputData}).bridgeName</code>).</li>
</ul>
<h2>Module LifeCycle</h2><p>Module methods are executed in this order:</p>
<ol>
<li><code>constructor()</code></li>
<li>if ModuleManager is provided by component &amp; .manager() exists -&gt; <code>.manager(manager)</code></li>
<li>if .integrate() exists -&gt;  <code>.integrate()</code></li>
<li>bridges are executed if such API is provided by components.</li>
</ol>
<blockquote>
<p>Only <code>constructor()</code> is executed once when you create a module instance. All other methods are executed once per component, where they are used.</p>
<p><strong>Therefore, you can use one module instance per multiple components in performance reasons (Recommended)</strong></p>
</blockquote>
<h2>Bridges provided by included components (WHS.Sphere, ...)</h2><table>
<thead>
<tr>
<th>Bridge name</th>
...
<td><code>THREE.Geometry</code> instance</td>
</tr>
<tr>
<td><code>material</code></td>
<td><code>THREE.Material</code> instance</td>
</tr>
<tr>
<td><code>mesh</code></td>
<td><code>THREE.Mesh</code> instance</td>
</tr>
</tbody>
</table>
<p><a href="Animation Clips.html">&gt; Animation Clips</a></p></article>
    </section>
